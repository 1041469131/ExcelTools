...
import com.hqjl.common.excel.adapter.ICellReaderAdapter;
...
public class ImportTableService {
    ...
    public <T> List<T> read(String[] sortedColumns, Class<? extends T> type) {
        ...
        //扩展列名导入
        if(tableBean.getColumnCount()>sortedColumns.length){
        ...
        for (int i = startRow+1; i < tableBean.getRowCount(); i++) {
            Object bean = Map.class.isAssignableFrom(type) ? new LinkedHashMap<> () : Reflections.instance(type.getName());
